...
import com.phonepe.cryptography.CryptAlgorithm;
import com.phonepe.cryptography.SimpleCrypto;
...
public class InMemoryStore {
    ...
    private CryptAlgorithm crypt = new SimpleCrypto();
    ...
    public synchronized void deleteFile(String fileName) throws FileNotFoundException {
        if (metadataStore.containsKey(fileName))
        {
            ...
            metadataStore.keySet().stream()
                    .map(metadataStore::get)
                    .filter(metadata -> metadata.getStartIdx() >= nextFileStartIdx)
                    .forEach(metadata -> metadata.setStartIdx(metadata.getStartIdx() - fileSize));
